      ...
      <div className="flex gap-4 flex-wrap mt-8">
        {toolAndTechnologies.map((item, index) => {
          ...
          return (
            !item.isDisabled && (
              <div
                className="bg-gray-200 rounded-sm flex p-4 w-48 justify-around gap-2 items-center cursor-pointer"
                onClick={() => redirectToDocs(item.docsLink)}
              >
