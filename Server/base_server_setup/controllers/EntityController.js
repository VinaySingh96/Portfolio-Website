...
const { multerUpload, FileUpload } = require("../utils/FileUpload");
...
class EntityController extends BaseController{
  ...
  loadRoutes() {
    if(this.crud.CREATE) this.router.post('/',this.multerUpload.array('files'), this.createEntity);
  ...
  async createEntity(req, res) {
    // try {
      const data = await this.service.createEntity(req.body, req.params, req.headers, req);
      res.send({
        data: data,
